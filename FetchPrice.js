...
//got the link from alchemy dashboard app api(ethereum mainnet) 
...
const fetchPrice = async(addressFrom, addressTo, humanValue) => {
	...
	//both functions from v3 protocol docs
	...
	const quotedAmountOut = await quoterContract.callStatic.quoteExactInputSingle(
	  ...
	  addressTo,
	  3000,
	  amountIn.toString(), //converting big number object(wei) to string
	  0
	);
...
const main = async() => {
	...
	const humanValue = '1'; //1 weth since weth is costly
